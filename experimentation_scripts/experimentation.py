...
# Train a GMM with k components, on a selected phoneme id which is stored in variable "p_id" 
...
# To fill X_phoneme, leverage the phoneme_id array, that contains the ID of each sample of X_full
...
for t in range(n_iter):
    ...
    # Do the E-step
    # Singularity solution 1 putting constraints on the covariance matrix s
    for i in range(len(s)):
        s[i] = s[i] * np.identity(3)

    # Singularity solution 2 regularizing by a
    #for i in range(len(s)):
        #s[i] = s[i] + np.array([[1,0,0], [0,1,0], [0,0,1]])
